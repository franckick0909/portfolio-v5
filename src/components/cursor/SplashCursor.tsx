...
export default function SplashCursor({
  ...
  TRANSPARENT = true,
  RAINBOW_MODE = false,
  COLOR = '#ffffff' // Blanc pour mix-blend-difference
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      gl.clearColor(0, 0, 0, 0); // Modifié pour être complètement transparent par défaut
    ...
    const blit = (() => {
      ...
      return (target: FBO | null, doClear = false) => {
        ...
        if (doClear) {
          gl.clearColor(0, 0, 0, 0); // Pour fond transparent
    ...
    function hexToRGB(hex: string): ColorRGB {
      ...
      // Normaliser pour WebGL mais plus fort que l'original
    ...
    // Démarrer la boucle !
    // (Ajout pour forcer l'update même si on ne bouge pas la souris tout de suite)
    animationFrameId = requestAnimationFrame(updateFrame);
